...
class GroundedAudioMultiscaleDeformableAttention(nn.Module):
    ...
    def forward(
        ...
        sampling_offsets = self.sampling_offsets(hidden_states).view(batch_size, num_queries, self.n_heads, self.n_points, 1)
        attention_weights = self.attention_weights(hidden_states).view(batch_size, num_queries, self.n_heads, self.n_points)
...
class GroundedAudioBiMultiHeadAttention(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, batch_size: int):
        tensor = tensor.view(batch_size, -1, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
        tensor = tensor.view(batch_size * self.num_heads, -1, self.head_dim)
        return tensor
    
    def _reshape(self, tensor: torch.Tensor, batch_size: int):
        tensor = tensor.view(batch_size, self.num_heads, -1, self.head_dim).transpose(1, 2)
        tensor = tensor.reshape(batch_size, -1, self.embed_dim)
        return tensor
    ...
    def forward(
        ...
        audio_query_states = self._shape(self.audio_proj(audio_features) * self.scale, batch_size)  # bs*nhead, audio_len, dim
        audio_value_states = self._shape(self.values_audio_proj(audio_features), batch_size)  # bs*nhead, audio_len, dim

        text_key_states = self._shape(self.text_proj(text_features), batch_size)  # bs*nhead, seq_len, dim
        text_value_states = self._shape(self.values_text_proj(text_features), batch_size)  # bs*nhead, seq_len, dim

        attn_weights = torch.bmm(audio_query_states, text_key_states.transpose(1, 2))  # bs*nhead, audio_len, seq_len

        attn_weights = attn_weights - attn_weights.max()
        attn_weights = torch.clamp(attn_weights, min=-50000, max=50000)  # Do not increase -50000/50000, data type half has quite limited range
        if text_attention_mask is not None:  # mask language for audio
        ...
        text_attn_weights = torch.clamp(text_attn_weights, min=-50000, max=50000)  # Do not increase -50000/50000, data type half has quite limited range
        if audio_attention_mask is not None:  # mask audio for language
            audio_attention_mask = audio_attention_mask[:, None, None, :].repeat(1, self.num_heads, 1, 1).flatten(0, 1)
        ...
        audio_attn_probs = F.dropout(audio_attn_weights, p=self.dropout, training=self.training)  # bs*nhead, audio_len, seq_len
        text_attn_probs = F.dropout(text_attn_weights, p=self.dropout, training=self.training)  # bs*nhead, seq_len, audio_len

        audio_attn_output = torch.bmm(audio_attn_probs, text_value_states)  # bs*nhead, audio_len, dim
        text_attn_output = torch.bmm(text_attn_probs, audio_value_states)  # # bs*nhead, seq_len, dim

        audio_attn_output = self._reshape(audio_attn_output, batch_size)
        text_attn_output = self._reshape(text_attn_output, batch_size)

        audio_attn_output = self.out_audio_proj(audio_attn_output)  # bs, audio_len, dim
        text_attn_output = self.out_text_proj(text_attn_output)  # bs, audio_len, dim
...
class GroundedAudioModel(GroundedAudioPreTrainedModel):
    ...
    def generate_encoder_output_proposals(self, enc_output, padding_mask):
        ...
        # proposal: [bs, time_len, 2], 每个时间点初始预测的事件中点和事件时长/2

        proposal_val = ((proposal > 0.01) & (proposal < 0.99)).all(-1, keepdim=True)
        proposal = torch.log(proposal / (1-proposal))  # inverse sigmoid. 因为此时proposal是0-1之间(相当于sigmoid归一化的预测值), 所以需要反向sigmoid, 得到原始预测值.
        proposal = proposal.masked_fill(padding_mask.unsqueeze(-1), torch.finfo(enc_output.dtype).max)
        proposal = proposal.masked_fill(~proposal_val, torch.finfo(enc_output.dtype).max) # 为了避免在图像边界的boundingbox预测.

        object_query = enc_output
        object_query = object_query.masked_fill(padding_mask.unsqueeze(-1), torch.tensor(0, dtype=enc_output.dtype))
        object_query = object_query.masked_fill(~proposal_val, torch.tensor(0, dtype=enc_output.dtype))
    ...
    def forward(
        ...
        if self.config.two_stage:
            ...
            # (bs, audio_len, dim), (bs, audio_len, 2)

            enc_outputs_class = self.encoder_output_class_embed(object_query_embedding, encoder_outputs[1], text_token_mask)  # (bs, audio_len, seq_len)
            delta_bbox = self.encoder_output_bbox_embed(object_query_embedding)  # (bs, audio_len, 2)

            enc_outputs_coord_logits = delta_bbox + output_proposals

            # 获取和文本存在最高相似度得分的audio token ids
            ...
            reference_points = topk_coords_logits.sigmoid()  # 归一化到0-1之间
